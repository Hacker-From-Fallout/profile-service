...
public interface CustomerProfileRepository extends JpaRepository<CustomerProfile, Long>, JpaSpecificationExecutor<CustomerProfile> {
    ...
    @Modifying
    @Query(value = "UPDATE customer_profiles p SET email = :email WHERE p.id = :id", nativeQuery = true)
    ...
    @Modifying
    @Query(value = "UPDATE customer_profiles u SET date_of_birth = :date_of_birth WHERE u.id = :id", nativeQuery = true)
    void updateDateOfBirth(@Param("id") Long id, @Param("date_of_birth") LocalDate dateOfBirth);

    @Modifying
    @Query(value = "UPDATE customer_profiles u SET gender = :gender WHERE u.id = :id", nativeQuery = true)
    void updateGender(@Param("id") Long id, @Param("gender") String gender);

    @Modifying
    @Query(value = "UPDATE customer_profiles u SET avatar_url = :avatar_url WHERE u.id = :id", nativeQuery = true)
